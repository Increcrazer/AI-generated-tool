function [R_bitperpulse, R_bitpersecond, e_obs, e_x1_U, q, nZ] = Decoy_Xu2015_corefunc ... 
            (epsilon_cor, epsilon_sec, ...
             qZ, N, f, gate_width, width_3dB, ...
             k, pk, ...
             L, alpha, eta_Bob_detect, eta_Bob_SPD, ...
    ...
    % eta_Bob_SPD Bob超导效率，1x2向量
    % e_mis_Z: A端态制备的Z基错误率，用Z基对比度反算得到（文章里假设phi0是完美的), f_EC: 纠错效率
    % p_ap: 后脉冲概率, dc_count[/s]: 超导单通道每秒暗计数，是个1x3向量（三态协议，三个超导）, deadtime[s]: 死时间
    % 输出：
...
%     Rk_x = Dk_x.*Cdt_x;  % B端探测每个强度态的概率(含死时间修正)
    ...
    [~, delta_theta] = calculate_delta_theta(D_theta, eta_Bob_SPD, 10.^(-10));
    ...
    n0x0zk = qZ.*(p_0x0z.*(1-exp(-eta_ch .*k)) + p_dc_X).*Cdt_x.*N;
    n0x1zk = qZ.*(p_0x1z.*(1-exp(-eta_ch .*k)) + p_dc_X).*Cdt_x.*N;
    n0x0xk = qX.*(p_0x0x.*(1-exp(-eta_ch .*k)) + p_dc_X).*Cdt_x.*N;
    
    n1x0zk = qZ.*(p_1x0z.*(1-exp(-eta_ch .*k)) + p_dc_X).*Cdt_x.*N;
    n1x1zk = qZ.*(p_1x1z.*(1-exp(-eta_ch .*k)) + p_dc_X).*Cdt_x.*N;
    n1x0xk = qX.*(p_1x0x.*(1-exp(-eta_ch .*k)) + p_dc_X).*Cdt_x.*N;
    
    
    %% 计算Z基比特误码率
    e_k_z = Pz.*pk.*(p_dc_Z./2 + e_mis_Z.*(1-exp(-eta_ch .*k))).*Cdt_z; % B端探测z基到且出错的概率
...
%% 
function [delta, delta_theta] = calculate_delta_theta(D_theta, eta, epsilon)
% 计算调制误差δ_θ的上界(式12)
%
% 输入参数:
%   D_theta - 一个nx2矩阵，其中D_theta(:,1)对应D_{1,θ}，D_theta(:,2)对应D_{2,θ}
%   eta - 探测器效率向量[η_{d1}, η_{d2}]
%   epsilon - 失败概率(如10.^-10)
...
%   delta_theta - 调制误差δ_θ的上界(弧度),是个1x2向量
    ...
    eta_d1 = eta(1);          % 探测器1的效率
    eta_d2 = eta(2);          % 探测器2的效率
    ...
    % 计算分子和分母项
    numerator = ((D1_theta + Delta_D1_theta) - (D1_0 - Delta_D1_0)) ./ eta_d1;
    denominator = ((D2_theta - Delta_D2_theta) - (D1_0 + Delta_D1_0)) ./ eta_d2;
...
function [X_basis_state, Z_basis_state, p_0x0z, p_0x1z, p_0x0x, p_1x0z, p_1x1z, p_1x0x] ...
    ...
    % 计算概率
    p_0x0z = (1./6) .* trace(D_0x .* rho0z);
    p_0x1z = (1./6) .* trace(D_0x .* rho1z);
    p_0x0x = (1./6) .* trace(D_0x .* rho0x);
    
    p_1x0z = (1./6) .* trace(D_1x .* rho0z);
    p_1x1z = (1./6) .* trace(D_1x .* rho1z);
    p_1x0x = (1./6) .* trace(D_1x .* rho0x);
end
...
function S_jx0x_1_plus = calculate_S_jx0x_1_plus(tau_1, mu_2, mu_3, njx0x_mu2_plus, njx0x_mu3_minus)
    S_jx0x_1_plus = tau_1 .* (njx0x_mu2_plus - njx0x_mu3_minus) ./ (mu_2 - mu_3);
end

function S_jx0x_1_minus = calculate_S_jx0x_1_minus(tau_0, tau_1, mu_1, mu_2, mu_3, ...
                            njx0xk_mu1_plus, njx0xk_mu2_plus, njx0xk_mu2_minus, njx0xk_mu3plus, njx0xk_mu3_minus)
    S_jx0x_0 = calculate_SZ_0(tau_0, mu_2, mu_3, njx0xk_mu2_plus, njx0xk_mu3_minus);
    S_jx0x_1_minus = calculate_SZ_1(tau_0, tau_1, mu_1, mu_2, mu_3, njx0xk_mu1_plus, njx0xk_mu2_minus, njx0xk_mu3plus, S_jx0x_0);
end
