...
% f_EC = 1.16; 
% pd = 7.2.*10.^-8;
% delta_max = [10^-8 10^-10 10^-12];
% 
% epsilon_cor = 10^-15;
% epsilon_sec = 10^-15;
% N = 10^40;
% e_mis_X = sin(delta_A)^2;
% e_mis_Z = e_mis_X;
% p_ap = 0;
% dc_count = [f*pd f*pd f*pd f*pd];
% deadtime = 0;
% qX = 0.95;
% 
% %%
% R_bitperpulse = zeros(length(delta_max),length(L));
% R_bitperpulse2 = zeros(1,length(L));
% for i = 1:length(delta_max)
%     for j = 1:length(L)
%         R_bitperpulse(i,j) = Decoy_Sixto2022MI_corefunc ... 
%                 (qZ, ncut, gate_width, width_3dB, ...
...
%                  delta_A, f_EC, ...
%                  pd, delta_max(i));
%         [R_bitperpulse2(j), ~, ~, ~, ~] = Decoy_Lim2014_corefunc ... 
...
% %% 绘图
% % Create figure with improved styling
% figure('Color', 'white', 'Position', [100, 100, 800, 600]); hold on;
% 
% % Plot with logarithmic y-axis
% semilogy(L, R_bitperpulse(1,:), 'LineWidth', 2.5, 'Color', [0, 0.4470, 0.7410], 'DisplayName', 'delta max = 1E-8'); hold on;
% semilogy(L, R_bitperpulse(2,:), 'LineWidth', 2.5, 'Color', [0.8500, 0.3250, 0.0980], 'DisplayName', 'delta max = 1E-10'); 
% semilogy(L, R_bitperpulse(3,:), 'LineWidth', 2.5, 'Color', [0.9290, 0.6940, 0.1250], 'DisplayName', 'delta max = 1E-12'); 
% semilogy(L, R_bitperpulse2, 'LineWidth', 2.5, 'Color', [0.9290, 0.6940, 0.1250], 'DisplayName', 'Lim2014'); 
% grid on;
% 
% % Add labels and title with larger fonts
% xlabel('Distance (km)', 'FontSize', 14, 'FontWeight', 'bold');
